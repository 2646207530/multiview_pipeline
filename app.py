...
def _det_frame_image(cam_idx: int, frame_idx: int):
    """单帧 bbox / wrist overlay (RGB ndarray). 不存在返回 None.

    优先吃 Step 2 落盘的 `_detect_vis/cam{ci}/<fid>.jpg` 缓存 (含 bbox + wrist 标记);
    没有就 fallback 到现场画 bbox (旧数据兼容, 不含 wrist).
    """
    if ctx.state is None:
        return None
    ud = ctx.state.steps.get("undistort")
    if ud is None or ud.status != "done":
        return None
    fid_str = f"{int(frame_idx):06d}"
    if ctx.workspace is not None:
        cached = ctx.workspace.detect_vis_dir / f"cam{int(cam_idx)}" / f"{fid_str}.jpg"
        if cached.is_file():
    ...
    o = ud.outputs
    p = (Path(o["undist_root"]) / o["capture_id"] / str(cam_idx) /
         "images_undistorted" / f"{fid_str}.jpg")
    if not p.exists():
        return None
    ...
    bboxes = (bbox_data.get(str(cam_idx)) or {}).get(fid_str, [])
    for entry in bboxes:
...
def _ckpt_dropdown_update(ws: Workspace, state: PipelineState):
    ...
    默认用 _DEFAULT_CKPT_LABEL; dropdown 允许自定义值, 想用别的 ckpt 直接填路径.
    """
    return gr.Dropdown(choices=[_DEFAULT_CKPT_LABEL], value=_DEFAULT_CKPT_LABEL)
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Golf Multi-View Pipeline") as demo:
        ...
        with gr.Tabs():
            ...
            # ── Step 2: Detection (ViTPose) ─────────────────────────
            with gr.Tab("2. Hand Detection"):
                det_backend = gr.Radio(
                    choices=["vitpose"], value="vitpose",
                    label="检测后端",
                    info="vitpose: 从 preprocess/<cam>/vitpose.pt 直接取 COCO17 手腕 "
                          "(9=L, 10=R) 当 bbox 中心 (固定 80px 方框, 不够大就调下面 "
                          "bbox size), conf<0.3 丢弃")
                ...
                btn_det.click(cb_detect,
                              [det_backend, det_force, det_bbox_size],
                              [out_det, status_box,
                               preview_det0, preview_det1, log_det,
                               det_frame, det_img0, det_img1])

            # ── Step 3: Pseudo Label (WiLoR) ──────────────────────
            with gr.Tab("3. Pseudo Label"):
                ps_backend = gr.Radio(
                    choices=["wilor"], value="wilor",
                    label="伪标后端",
                    info="wilor: 用 step2 的 bbox 估计 21 个 2D 关节, 写 npz")
            ...
            # ── Step 4: Multi-view Inference ───────────────────────
            with gr.Tab("4. Multi-view Inference + npy"):
                with gr.Row():
                    ckpt_dd = gr.Dropdown(
                        label="Checkpoint",
                        info="默认用 exp/new/checkpoints/checkpoint_30; "
                             "想换权重直接在框里填 ckpt 目录路径.",
            ...
            # ── Step 5: Visualization (way_vis) ────────────────────
            with gr.Tab("5. 3D Visualization (way_vis)"):
