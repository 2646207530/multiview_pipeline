...
#
# 注意 (golf_pipeline 改): 原版 SAM2 钉的下限太激进 (torch>=2.5.1,
# numpy>=1.24.4, pillow>=9.4.0), `pip install -e .` 会强行升级父环境的:
#   - torch 2.1.2+cu118 → 2.12.0+cu130 (老 NVIDIA 驱动直接 CUDA init 失败)
#   - numpy 1.23.x → 2.2.x (torch 2.1.2 按 numpy 1.x ABI 编, 报 'Numpy is not available')
#   - Pillow 10.x → 12.x (新 libLerc 要 GLIBCXX_3.4.29, 系统旧 libstdc++ 没有)
# 这里把下限松到能跑就行, 跟 golf_pipeline 的 requirements.txt 对齐.
# SAM2 runtime 实际不依赖那些上限版本的特性, 旧版完全跑得通.
REQUIRED_PACKAGES = [
    "torch>=2.1",
    "torchvision>=0.16",
    "numpy>=1.21,<2",
