...
class TempoTransform2D:
    ...
    def __init__(self, cfg: CN) -> None:
        ...
        self._aug = cfg.AUG
        self._seq = cfg.SEQ
        self.bbox_3d_size = cfg.get("BBOX_3D_SIZE", 0.4)
    ...
    def __call__(self, image, label):
        target_joints_2d = [None for _ in range(self._seq)]
        target_joints_vis = [None for _ in range(self._seq)]
        bbox_center = [None for _ in range(self._seq)]
        bbox_scale = [None for _ in range(self._seq)]

        affine = [None for _ in range(self._seq)]
        if self._aug:
            ...
            for i in range(self._seq):
                bbox_center[i] = label["bbox_center"][i] + c_factor * label["bbox_scale"][i]
                bbox_scale[i] = label["bbox_scale"][i] * s_factor
                if self._occlusion:  # apply occlu
                    occlu_inp = {}
                    occlu_inp["bbox"] = center_scale_to_box(bbox_center[i], bbox_scale[i])
                    occlu_inp["width"], occlu_inp["height"] = image[i].shape[1], image[i].shape[0]
                    occlu_inp["image"] = image[i]
                    image[i] = self.occlusion_op(occlu_inp)["image"]
            # r_factor = 60
        ...
        for i in range(self._seq):
            affine[i] = _affine_transform(center=bbox_center[i], scale=bbox_scale[i], out_res=self._output_size, rot=rot)
            target_joints_2d[i] = _transform_coords(label["joints_2d"][i], affine[i]).astype(np.float32)

            jv = label["joints_vis"][i]
            if not self._train:
                target_joints_vis[i] = np.full(CONST.NUM_JOINTS, 1.0, dtype=np.float32)
            elif jv.sum() < CONST.NUM_JOINTS * 0.3:
                target_joints_vis[i] = np.full(CONST.NUM_JOINTS, 0.0, dtype=np.float32)
            else:
                tj2d = target_joints_2d[i]
                target_joints_vis[i] = (((tj2d[:, 0] >= 0) & (tj2d[:, 0] < self._output_size[0])) &
                                    ((tj2d[:, 1] >= 0) & (tj2d[:, 1] < self._output_size[1]))).astype(np.float32)
                if target_joints_vis[i].sum() < CONST.NUM_JOINTS * 0.3:  # emprical number
                    target_joints_vis[i] = np.full(CONST.NUM_JOINTS, 0.0, dtype=np.float32)

            affine_2x3 = affine[i][:2, :]
            image[i] = cv2.warpAffine(image[i],
            ...
            if self._aug:
                ...
                image[i][:, :, 0] = np.clip(image[i][:, :, 0] * random.uniform(c_low, c_high), 0, 255)
                image[i][:, :, 1] = np.clip(image[i][:, :, 1] * random.uniform(c_low, c_high), 0, 255)
                image[i][:, :, 2] = np.clip(image[i][:, :, 2] * random.uniform(c_low, c_high), 0, 255)

            # image_np[i] = image[i]
            image[i] = tvF.to_tensor(image[i])
            assert image[i].shape[0] == 3
            image[i] = tvF.normalize(image[i], [0.5, 0.5, 0.5], [1, 1, 1])
            image[i] = image[i].numpy()
...
@TRANSFORM.register_module()
class TempoTransformUVD(TempoTransform2D):
    ...
    def __call__(self, image, label):
        ...
        pj_uv = [None for _ in range(self._seq)]
        dj_uv = [None for _ in range(self._seq)]
        root_joint_d = [None for _ in range(self._seq)]
        target_joints_uvd = [None for _ in range(self._seq)]
        target_verts_uvd = [None for _ in range(self._seq)]
        raw_size = label["raw_size"]
        affine = results["affine"]
        joints_uvd = label["joints_uvd"]
        verts_uvd = label["verts_uvd"]

        results["joints_ori"] = joints_uvd

        for i in range(self._seq):
            joints_uv = joints_uvd[i][:, :2]  # (21, 2)
            verts_uv = verts_uvd[i][:, :2]  # (778, 2)
            joints_uv = _transform_coords(joints_uv, affine[i]).astype(np.float32)
            # joints_uv_post = _transform_coords(joints_uv, affine[i], invert=True).astype(np.float32)
            verts_uv = _transform_coords(verts_uv, affine[i]).astype(np.float32)

            joints_d = joints_uvd[i][:, 2:]  # (21, 1)
            verts_d = verts_uvd[i][:, 2:]  # (778, 1)
            root_joint_d[i] = joints_d[self._center_idx].copy()  # (1)

            joints_d_rel = joints_d - root_joint_d[i]
            verts_d_rel = verts_d - root_joint_d[i]

            # tj_uv = joints_uv / np.array([self._output_size[0], self._output_size[1]])  # (21, 2)
            tj_uv = joints_uv / np.array([self._output_size[0] / 2, self._output_size[1] / 2]) - 1 # (21, 2)
            # tt_uv = (tj_uv + 1) * np.array([self._output_size[0] / 2, self._output_size[1] / 2])
            # joints_uv_post = _transform_coords(tt_uv, affine[i], invert=True).astype(np.float32)
            tj_d = 0.5 + (joints_d_rel[:, :1] / CONST.UVD_DEPTH_RANGE)  # (21, 1)
            target_joints_uvd[i] = np.concatenate([tj_uv, tj_d], axis=1).astype(np.float32)

            # tv_uv = verts_uv / np.array([self._output_size[0], self._output_size[1]])  # (778, 2)
            tv_uv = verts_uv / np.array([self._output_size[0] / 2, self._output_size[1] / 2]) - 1  # (778, 2)
            tv_d = 0.5 + (verts_d_rel[:, :1] / CONST.UVD_DEPTH_RANGE)  # (21, 1)
            target_verts_uvd[i] = np.concatenate([tv_uv, tv_d], axis=1).astype(np.float32)

            if 'pseudo_2d' in label:
                pseudo_uv_ori = label['pseudo_2d'][i]
                pseudo_uv = _transform_coords(pseudo_uv_ori, affine[i]).astype(np.float32)
                pj_uv[i] = (pseudo_uv / np.array([self._output_size[0] / 2, self._output_size[1] / 2]) - 1).astype(np.float32)  # (21, 2)

            if 'dyn_2d' in label:
                dyn_uv = label['dyn_2d'][i]
                dyn_uv = _transform_coords(dyn_uv, affine[i])
                dj_uv[i] = (dyn_uv / np.array([self._output_size[0] / 2, self._output_size[1] / 2]) - 1).astype(np.float32)  # (21, 2)
                
        results['target_dyn_uv'] = dj_uv    
        results['target_pseudo_uv'] = pj_uv
        results["target_root_d"] = root_joint_d
        results["target_joints_uvd"] = target_joints_uvd
        results["target_verts_uvd"] = target_verts_uvd
        # results["joints_ori"] = joints_uv

        return results


@TRANSFORM.register_module()
class TempoTransform3DMultiView(TempoTransformUVD):
    ...
    def __call__(self, image, label):
        results = super().__call__(image, label)
        affine_postrot = [None for _ in range(self._seq)]
        target_cam_extr = [None for _ in range(self._seq)]
        target_cam_intr = [None for _ in range(self._seq)]
        rotation = [None for _ in range(self._seq)]
        target_joints_3d = [None for _ in range(self._seq)]
        target_verts_3d = [None for _ in range(self._seq)]
        target_joints_3d_rel = [None for _ in range(self._seq)]
        target_verts_3d_rel = [None for _ in range(self._seq)]
        target_root_joint = [None for _ in range(self._seq)]
        target_joints_3d_no_rot = [None for _ in range(self._seq)]
        target_verts_3d_no_rot = [None for _ in range(self._seq)]
        rot = results["rot_rad"]
        rot_mat = results["rot_mat3d"]

        for i in range(self._seq):
            center = results["target_bbox_center"][i]
            scale = results["target_bbox_scale"][i]
            cc = label["cam_center"][i]
            affine_postrot[i] = _affine_transform_post_rot(center=center,
                                                        scale=scale,
                                                        optical_center=cc,
                                                        out_res=self._output_size,
                                                        rot=rot)

            target_cam_intr[i] = affine_postrot[i].dot(label["cam_intr"][i])
            target_cam_extr[i] = label["cam_extr"][i]
            rotation[i] = rot_mat.dot(target_cam_extr[i][:3, :3])
            target_joints_3d_no_rot[i] = label["joints_3d"][i] / (self.bbox_3d_size / 2) 
            target_verts_3d_no_rot[i] = label["verts_3d"][i] / (self.bbox_3d_size / 2)
            target_joints_3d[i] = rot_mat.dot(label["joints_3d"][i].transpose(1, 0)).transpose() / (self.bbox_3d_size / 2)
            target_verts_3d[i] = rot_mat.dot(label["verts_3d"][i].transpose(1, 0)).transpose() / (self.bbox_3d_size / 2)
            target_root_joint[i] = target_joints_3d[i][self._center_idx]
            target_joints_3d_rel[i] = target_joints_3d[i] - target_root_joint[i]
            target_verts_3d_rel[i] = target_verts_3d[i] - target_root_joint[i]

        results["affine_postrot"] = affine_postrot
        # results["extr_prerot"] = rot_mat
        results["target_cam_intr"] = target_cam_intr
        results["target_cam_extr"] = target_cam_extr
...
@TRANSFORM.register_module()
class TempoTransform3D(TempoTransformUVD):
...
@TRANSFORM.register_module()
class TempoTransform3DMANO(TempoTransform3D):
...
def batch_transform_coords(pts, affine_trans, invert=False):
    """
    Args:
        pts: (batch_size, point_nb, 2)
        affine_trans: (batch_size, 3, 3)
    """
    affine_trans = affine_trans.to(pts.device)
    if invert:
        affine_trans = torch.inverse(affine_trans)
    
    # Convert to homogeneous coords: (batch_size, point_nb, 3)
    hom2d = torch.cat([pts, torch.ones(*pts.shape[:-1], 1, device=pts.device)], dim=-1)
    
    # Apply transform: (batch_size, point_nb, 3) -> (batch_size, point_nb, 2)
    transformed = torch.einsum('bij,bpj->bpi', affine_trans, hom2d)[..., :2]
    return transformed


def batch_save_transformed_coords_json(
    save_path,
    batch,
    update_pgt,
    update_conf,
    ...
    """
    Save transformed points as JSON files in batch.

    """
    affine = batch['affine'][:, :, 1].flatten(0, 1)
    # ori = batch['joints_ori'].view(-1, 21, 3)
    # gt = batch['target_joints_uvd'].view(-1, 21, 3)
    file_num = batch['file_num'].flatten()
    cam_num = batch['cam_num'].flatten()
    frame_num = batch['frame_num'].flatten()
    # pseudo_hm = (preds['pred_joint_uv_hmap'] + 1) * 128
    # pseudo_hm = pseudo_hm.reshape(-1, 8, 3, 21, 2)[:, :, 1].flatten(0, 1)
    pseudo_unc = (update_pgt + 1) * 128
    pseudo_unc = pseudo_unc
    # gto = (gt[:, :, :2] + 1) * 128
    # hm_cfd = preds['pred_cfd'].reshape(-1, 8, 3, 21)[:, :, 1].flatten(0, 1)
    unc_cfd = update_conf.reshape(-1, 21)
    # reversed_hm = batch_transform_coords(pseudo_hm, affine, True)
    reversed_unc = batch_transform_coords(pseudo_unc, affine, True)
    # reversed_gt = batch_transform_coords(gto, affine, True)
    
    for i in range(reversed_unc.shape[0]):
        # pts_hm_i = reversed_hm[i].flatten().detach().cpu().tolist()
        pts_unc_i = reversed_unc[i].flatten().detach().cpu().tolist()
        # hm_cfd_i = hm_cfd[i].flatten().detach().cpu().tolist()
        unc_cfd_i = unc_cfd[i].flatten().detach().cpu().tolist()

        fn = file_num[i].detach().cpu()
        cn = cam_num[i].detach().cpu()
        fan = frame_num[i].detach().cpu()
        # save_root_hm = os.path.join(save_path, 'Dynamic_pseudo_hm', f"epoch{epoch}")
        save_root_unc = os.path.join(save_path, 'Dynamic_pseudo_unc', f"epoch{epoch}")
        # os.makedirs(save_root, exist_ok=True)
        # output_path_hm = os.path.join(save_root_hm, f"{fn:04d}", f"cam{cn}")
        output_path_unc = os.path.join(save_root_unc, f"{fn:04d}", f"cam{cn}")
        # os.makedirs(output_path_hm, exist_ok=True)
        os.makedirs(output_path_unc, exist_ok=True)
        # with open(os.path.join(output_path_hm, f"{fan:08d}.json"), "w") as f:
        #     json.dump(
        #         [
        #             pts_hm_i,
        #             hm_cfd_i
        ...
        with open(os.path.join(output_path_unc, f"{fan:08d}.json"), "w") as f:
            json.dump(
                [
                    pts_unc_i,
                    unc_cfd_i
