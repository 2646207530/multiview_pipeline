...
class SimpleTransform2D:
    ...
    def __call__(self, image, label):
        ...
        pse_joints_2d = _transform_coords(label["pseudo_2d"], affine).astype(np.float32)
        ...
        if not self._train:
            ...
            if pse_joints_vis.sum() < CONST.NUM_JOINTS * 0.3:  # emprical number
                pse_joints_vis = np.full(CONST.NUM_JOINTS, 0.0, dtype=np.float32)
        ...
        if self._with_heatmap:
            ...
            target_pj_heatmap = np.zeros((npJ, self._heatmap_size[1], self._heatmap_size[0]),
                                        dtype="float32")  # (nJ, H, W)
            ...
            results["pse_joints_heatmap"] = target_j_heatmap
...
class SimpleTransform2D_HanCo:
    ...
    def __call__(self, image, label):
        ...
        target_joints_2d = _transform_coords(label["joints_2d"], affine).astype(np.float32)
        pse_joints_2d = _transform_coords(label["pse_kpts_2d"], affine).astype(np.float32)
...
@TRANSFORM.register_module()
class SimpleTransformUVD_HanCo(SimpleTransform2D_HanCo):
    ...
    def __call__(self, image, label):
        ...
        # pj_uv = pseudo_uv / np.array([self._output_size[0], self._output_size[1]])  # (21, 2)
        pj_uv = pseudo_uv / np.array([self._output_size[0] / 2, self._output_size[1]/ 2]) - 1  # (21, 2)
        # pj_uv = np.hstack((pj_uv, np.ones((21, 1)))).astype(np.float32)
...
class SimpleTransform3DMultiView(SimpleTransformUVD):
    ...
    def __call__(self, image, label):
        ...
        pse_joints_3d = label['pse_joints']
        pse_verts_3d = label['pse_verts']
        ...
        results["pse_joints_3d"] = pse_joints_3d
        results["pse_verts_3d"] = pse_verts_3d
...
@TRANSFORM.register_module()
class SimpleTransform3DMultiView_HanCo(SimpleTransformUVD_HanCo):
    ...
    def __call__(self, image, label):
        ...
        target_cam_intr = np.dot(np.concatenate((affine, np.array([[0, 0, 1]], dtype=np.float32)), axis=0), label["cam_intr"])
        target_cam_extr = label["cam_extr"]
        rotation = target_cam_extr[:3, :3]
        # pse_joints_3d_no_rot = label["pse_kpts"]
        # pse_verts_3d_no_rot = label["pse_verts"]
        # target_joints_3d_no_rot = label["joints_3d"]
        # target_verts_3d_no_rot = label["verts_3d"]
        pse_joints_3d = label["pse_joints"]
        pse_verts_3d = label["pse_verts"]
        target_joints_3d = label["joints_3d"]
        target_verts_3d = label["verts_3d"]
        # target_verts_3d = rot_mat.dot(label["verts_3d"].transpose(1, 0)).transpose()
        # pse_root_joint = pse_joints_3d[self._center_idx]
        # pse_joints_3d_rel = pse_joints_3d - pse_root_joint
        # pse_verts_3d_rel = pse_verts_3d - pse_root_joint
        target_root_joint = target_joints_3d[self._center_idx]
        target_joints_3d_rel = target_joints_3d - target_root_joint
        # pse_joints_3d_target_root = pse_joints_3d_rel + target_root_joint
        # pse_verts_3d_target_root = pse_verts_3d_rel + target_root_joint
        target_verts_3d_rel = target_verts_3d - target_root_joint
        ...
        results["target_joints_3d"] = target_joints_3d
        results["target_verts_3d"] = target_verts_3d
        # results["pse_joints_3d_target_root"] = pse_joints_3d_target_root
        # results["target_verts_3d"] = pse_verts_3d_target_root
        # results["pse_joints_3d_rel"] = pse_joints_3d_rel
        # results["pse_verts_3d_rel"] = pse_verts_3d_rel
        results["target_joints_3d_rel"] = target_joints_3d_rel
        results["target_verts_3d_rel"] = target_verts_3d_rel 
        results["target_root_joint"] = target_root_joint

        # # for extr prerot
        # results["pse_joints_3d_no_rot"] = pse_joints_3d_no_rot
        # results["pse_verts_3d_no_rot"] = pse_verts_3d_no_rot
        # results["target_joints_3d_no_rot"] = target_joints_3d_no_rot 
        # # results["target_verts_3d_no_rot"] = target_verts_3d_no_rot
...
@TRANSFORM.register_module()
class TestTransformUVD(TestTransform):
...
@TRANSFORM.register_module()
class TestTransform3DMultiView(TestTransformUVD):
...
@TRANSFORM.register_module()
class InferenceTransformUVD(InferenceTransform):
...
@TRANSFORM.register_module()
class InferenceTransform3DMultiView(InferenceTransformUVD):
...
#         # rot_mat3d = _construct_rotation_matrix(rot)
#         # # affine = _affine_transform(center=bbox_center, scale=bbox_scale, out_res=self._output_size, rot=r_factor)
#         # affine, affine_inv = gen_trans_from_patch_cv(bbox_center, bbox_scale, self._output_size, rot, (0, 0))
...
#         pj_uv = pseudo_uv / np.array([self._output_size[0], self._output_size[1]]).astype(np.float32)  # (21, 2)
