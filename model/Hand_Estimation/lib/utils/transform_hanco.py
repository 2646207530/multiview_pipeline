...
class SimpleTransform2D:
    ...
    def __init__(self, cfg: CN) -> None:
        ...
        self._translation_jit_factor = cfg.get("TRANSL_JIT", 0 if self._aug else 0)
    ...
    def __call__(self, image, label):
        if self._aug:
            tf = self._translation_jit_factor
            cf = self._center_jit_factor
            sf = self._scale_jit_factor
            rf = self._rot_jit_factor
            transl_x = (np.random.rand() * 2 - 1) * tf
            transl_y = (np.random.rand() * 2 - 1) * tf
            ...
            rot = np.clip(np.random.normal(loc=0, scale=rf), -3 * rf, 3 * rf) if np.random.rand() <= self._rot_prob else 0.0
        ...
        affine, affine_inv = gen_trans_from_patch_cv(bbox_center, bbox_scale, self._output_size, rot, transl)
        ...
        image = cv2.warpAffine(image,
                               affine, (int(self._output_size[0]), int(self._output_size[1])),
...
class SimpleTransformUVD(SimpleTransform2D):
    ...
    def __call__(self, image, label):
        ...
        # if 'dyn_2d' in label:
        #     dyn_uv = label['dyn_2d']
        #     dyn_uv = _transform_coords(dyn_uv, affine)
        #     dj_uv = dyn_uv / np.array([self._output_size[0] / 2, self._output_size[1] / 2]) - 1  # (21, 2)
        #     results['target_dyn_uv'] = dj_uv.astype(np.float32)
...
class SimpleTransform3DMultiView(SimpleTransformUVD):
    ...
    def __call__(self, image, label):
        ...
        # scale = results["target_bbox_scale"]
        # cc = label["cam_center"]
        # affine_postrot = _affine_transform(center, scale, self._output_size)
        # affine_postrot = _affine_transform_post_rot(center=center,
        #                                             scale=scale,
        #                                             optical_center=cc,
        #                                             out_res=self._output_size,
        ...
        target_cam_intr = label["cam_intr"]
        target_cam_intr = np.dot(np.concatenate((affine, np.array([[0, 0, 1]], dtype=np.float32)), axis=0), target_cam_intr.copy())
        target_cam_extr = label["cam_extr"]
        rotation = rot_mat.dot(target_cam_extr[:3, :3])
        target_joints_3d_no_rot = label["joints_3d"]
        target_verts_3d_no_rot = label["verts_3d"]
        target_joints_3d = rot_mat.dot(label["joints_3d"].transpose(1, 0)).transpose()
        target_verts_3d = rot_mat.dot(label["verts_3d"].transpose(1, 0)).transpose()
...
def _construct_rotation_matrix(rot, size=3):
    ...
    if rot != 0:
        sn, cs = np.sin(np.deg2rad(rot)), np.cos(np.deg2rad(rot))
        rot_mat[0, :2] = [cs, -sn]
        rot_mat[1, :2] = [sn, cs]
...
def gen_trans_from_patch_cv(bbox_center, bbox_scale, out_res, rot, transl):
    # augment size with scale
    src_w = bbox_scale
    src_h = bbox_scale
    src_center = bbox_center
...
def batch_save_transformed_coords_json(
    ...
    """
    affine = batch['affine'].flatten(0, 1)
    ...
    pseudo_unc = pseudo_unc.reshape(-1, 8, 21, 2).flatten(0, 1)
    # gto = (gt[:, :, :2] + 1) * 128
    # hm_cfd = preds['pred_cfd'].reshape(-1, 8, 3, 21)[:, :, 1].flatten(0, 1)
    unc_cfd = update_conf.reshape(-1, 8, 21).flatten(0, 1)
    ...
    for i in range(reversed_unc.shape[0]):
        ...
        save_root_unc = os.path.join(save_path, 'Dynamic_pseudo')
