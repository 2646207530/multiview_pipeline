...
def batch_triangulate_dlt_cfd_torch(kp2ds, Ks, Extrs, confidences):
    """Triangulate 3D points with confidence-weighted DLT.
    Args:
        kp2ds (torch.Tensor): (B, N, J, 2) - 2D keypoints.
        Ks (torch.Tensor): (B, N, 3, 3) - Camera intrinsics.
        Extrs (torch.Tensor): (B, N, 4, 4) - Extrinsics [R|t].
        confidences (torch.Tensor): (B, N, J) - Keypoint confidences (0-1).
    Returns:
        torch.Tensor: (B, J, 3) - Triangulated 3D points.
    """
    batch_size, nCams, nJoints, _ = kp2ds.shape
    Pmat = Extrs[..., :3, :]  # (B, N, 3, 4)
    Mmat = torch.matmul(Ks, Pmat)  # (B, N, 3, 4)
    
    # Expand Mmat to (B, J, N, 3, 4)
    Mmat = Mmat.unsqueeze(1).repeat(1, nJoints, 1, 1, 1)  # (B, J, N, 3, 4)
    Mmat = Mmat.reshape(batch_size * nJoints, nCams, *Pmat.shape[-2:])  # (BxJ, N, 3, 4)
    
    # Get rows for DLT construction
    M_row2 = Mmat[..., 2:3, :]  # (BxJ, N, 1, 4)
    
    # Reshape kp2ds to (BxJ, N, 2, 1)
    kp2ds = kp2ds.permute(0, 2, 1, 3).reshape(batch_size * nJoints, nCams, 2).unsqueeze(3)
    
    # Reshape confidences to (BxJ, N, 1, 1)
    confidences = confidences.permute(0, 2, 1).reshape(batch_size * nJoints, nCams, 1, 1)
    confidences = torch.clamp(confidences, min=1e-6, max=1.0)
    
    # Compute weighted DLT equations
    A = kp2ds * M_row2 - Mmat[..., :2, :]  # (BxJ, N, 2, 4)
    
    # Apply confidence weights: A[i,j] *= conf[i,j]
    A = A * confidences  # (BxJ, N, 2, 4)
