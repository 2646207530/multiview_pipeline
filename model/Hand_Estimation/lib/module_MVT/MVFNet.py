...
from lib.utils.builder import HEAD
...
from lib.utils.points_utils import sample_points_from_ball_query
from lib.models.bricks.transformer import build_transformer
from lib.models.layers.petr_transformer import SinePositionalEncoding3D
...
class MultiViewFusionNet(BasePointEmbedHead):
    def __init__(self, cfg: CN):
        ...
        # MVP: g + I
        # METRO: g cat V_tmpl
        super(MultiViewFusionNet, self).__init__(cfg)

    def _build_head_module(self):
        super(MultiViewFusionNet, self)._build_head_module()
    ...
    def generate_query(self, reference_embed, reference_points, template_mesh):
    ...
    def forward(self, inputs, mlvl_feat, img_metas, reference_points, template_mesh, **kwargs):
        results = dict()
        mlvl_feat = inputs['mlvl_feat']
        inp_img_w, inp_img_h = inputs["inp_img_shape"]  #  (256, 256)
        x = mlvl_feat
        batch_size, num_cams = x.size(0), x.size(1)
        pre_joints_2d = inputs['keypoints_2d']
        uv_coord_im = (pre_joints_2d + 0.5) * 256
        uv_coord_im = uv_coord_im.view(batch_size, num_cams, 21, 2)  # (B, 21, 2)
        K = inputs['target_cam_intr']
        T_c2m = torch.linalg.inv(inputs['target_cam_extr'])
        ref_joints = batch_triangulate_dlt_torch(uv_coord_im, K, T_c2m)
        ref_joints_root = ref_joints[:, self.center_idx, :].unsqueeze(1)
        ref_joints_rel = ref_joints - ref_joints_root
        ...
        # position embedding
        coords_embed, _, _, _ = self.position_embeding(mlvl_feat, inputs, masks)
        ...
        reference_embed = self.reference_embed.weight  # (21, 256)
