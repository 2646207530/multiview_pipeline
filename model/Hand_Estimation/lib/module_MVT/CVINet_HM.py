...
class CrossViewInteractNet(nn.Module):
    ...
    def get_inputs_log_prob(self, ctx:Tensor, UV:Tensor) -> Tuple[Tensor]:
        ...
        log_p = self.flow.log_prob(UV.reshape(BTNJ*self.selected_samples, -1), ctx.reshape(BTNJ*self.selected_samples, -1))
    ...
    def generate_random_samples(self, ctx:Tensor, num_samples:int) -> Tuple[Tensor]:
        ...
        samples = self.flow.forward_p(z.reshape(BTNJ*num_samples, -1), ctx.unsqueeze(1).repeat(1, num_samples, 1).reshape(BTNJ*num_samples, -1))
        samples = samples.reshape(BTNJ, num_samples, 2)
        log_prob = self.flow.log_prob(z.reshape(BTNJ*num_samples, -1), ctx.unsqueeze(1).repeat(1, num_samples, 1).reshape(BTNJ*num_samples, -1))
    ...
    def forward(self, batch, inputs):
        ...
        jaf = F.grid_sample(mlvl_feat.flatten(0, 1), uvc.detach().unsqueeze(2), align_corners=False)[..., 0].permute(0, 2, 1)
        ...
        all_feats = all_feats + self.att_0(all_feats, conf.detach().reshape(-1, N*J, 1)) + master_feats  # (BT, NJ, feat_dim)
        ...
        all_feats = all_feats + self.att_1(all_feats, conf.detach().reshape(-1, N*J, 1)) + avg_feats  # (BT, NJ, feat_dim)
        ...
        anchor_im = (anchor_uv.reshape(B*T, N, J, 2) + 1) * H / 2
        ref_joints = batch_triangulate_dlt_cfd_torch(anchor_im, K, T_c2m, conf.reshape(B*T, N, J))   # (B, J, 3)

        ambig_uv = pred_uv[:, 1:self.selected_samples].reshape(BTN, J*(self.selected_samples - 1), 2)
        ambig_im = (ambig_uv.reshape(B*T, N, J*(self.selected_samples - 1), 2) + 1) * H / 2
        ambig_conf = conf.unsqueeze(2).repeat(1, 1, self.selected_samples - 1, 1)
        ambig_ref_joints = batch_triangulate_dlt_cfd_torch(ambig_im, K, T_c2m, ambig_conf.reshape(B*T, N, J*(self.selected_samples - 1)))
