...
from lib.utils.triangulation import batch_triangulate_dlt_cfd_torch, batch_triangulate_dlt_torch
...
class SingleViewNet(nn.Module):
    def __init__(self, cfg: CN, cfg_preset: CN):
        ...
        self.feat_dim = self.saigb_dim  # 可配置化
        ...
        self.gcn = GraphRegression(self.joint_num, self.feat_dim, self.feat_dim, last=False)
        self.attn = SelfAttn(self.feat_dim, self.feat_dim, dropout=0)
    ...
    def forward(self, batch):
        ...
        g_features = self.gcn(features)  # (BN, J, feat_dim)
        features = self.attn(features.reshape(-1, N*J, self.feat_dim)).reshape(-1, J, self.feat_dim) + g_features  # (BN, J, feat_dim)
        features = self.fuse(features)  # (BN, J, out_dim)
        fu = features.view(-1, J*self.out_dim)  # (BN, J*out_dim)
        ...
        # out_sigma = self.fc_sigma(pred_hand_pose).reshape(BN, J, -1)
        ...
        ref_joints = (coord_uv[:, 778:].reshape(B, N, J, 2) + 1) * (H // 2)
        ref_joints_master = batch_triangulate_dlt_torch(ref_joints, K, T_c2m)   # (B, J, 3)
        # ref_joints_master = batch_triangulate_dlt_cfd_torch(ref_joints, K, T_c2m, scores.detach().view(B, N, J))   # (B, J, 3)
        ...
        #     # (B, K, 2)
        #     log_phi = self.flow.log_prob(bar_mu.reshape(-1, 2)).reshape(BN, J, 1)
