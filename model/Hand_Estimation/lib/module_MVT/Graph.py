...
class SelfAttnWithConfidence(nn.Module):
    def __init__(self, f_dim, hid_dim=None, n_heads=4, d_q=None, d_v=None, num_view=8, dropout=0.1):
        ...
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.w_qs = nn.Linear(f_dim + 1, n_heads * d_q)
        self.w_ks = nn.Linear(f_dim + 1, n_heads * d_q)
        self.w_vs = nn.Linear(f_dim + 1, n_heads * d_v)
        self.layer_norm = nn.LayerNorm(f_dim + 1, eps=1e-6)
        ...
        if mask and self.training:
            ...
            for view_idx in range(self.num_view):
                ...
                for use_idx in use_view_set:
                    mask[view_idx*joint_num:(view_idx+1)*joint_num, use_idx*joint_num:(use_idx+1)*joint_num] = float('-inf')
            mask = mask.unsqueeze(0).unsqueeze(1).repeat(attn.shape[0], attn.shape[1], 1, 1)
            attn = attn + mask.to(x.device)
        
        # 原有valid掩码机制（过滤完全无效的关键点）
    ...
    def forward(self, x, conf, valid=None, mask=False):
        BS, V, f = x.shape
        assert f == self.f_dim

        x_c = torch.cat([x, conf], dim=-1)  # 在特征维度上拼接置信度信息
        x = x + self.self_attn(self.layer_norm(x_c), valid, mask)
        # 前馈网络
        x = self.ff(x)
