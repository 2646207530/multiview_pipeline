...
class PAEval(Metric):
    ...
    def __init__(self, cfg, name="", mesh_score=False) -> None:
        super().__init__()
        ...
        self.name_pa_mpjpe = f"{name}PM"
        self.name_mpjpe = f"{name}M"
    ...
    def get_measures(self, **kwargs) -> Dict[str, float]:
        measures = {}
        measures[f"{self.name_pa_mpjpe}"] = self.pa_mpjpe.avg
        measures[f"{self.name_mpjpe}"] = self.mpjpe.avg
        if self.mesh_score:
            measures[f"{self.name_pa_mpjpe}"] = self.pa_mpvpe.avg
            measures[f"{self.name_mpjpe}"] = self.mpvpe.avg
        return measures

    def get_result(self):
        if not self.mesh_score:
            return self.pa_mpjpe.avg, self.mpjpe.avg
        else:
            return self.pa_mpjpe.avg, self.mpjpe.avg, self.pa_mpvpe.avg, self.mpvpe.avg

    def __str__(self) -> str:
        score = f"{self.name_pa_mpjpe}(mm): {self.pa_mpjpe.avg * 1000.0 :6.4f} | {self.name_mpjpe}(mm): {self.mpjpe.avg * 1000.0 :6.4f}"
        if self.mesh_score:
            score += f" | {self.name_pa_mpjpe}(mm): {self.pa_mpvpe.avg * 1000.0:6.4f} | {self.name_mpjpe}(mm): {self.mpvpe.avg * 1000.0 :6.4f}"
...
class PAEval_new(Metric):
    ...
    def __init__(self, cfg, name="", mesh_score=False) -> None:
        ...
        self.pa_mpjpe = AverageMeter()
        self.mpjpe = AverageMeter()
        self.name_pa_mpjpe = f"{name}_PJ"
        self.name_mpjpe = f"{name}_J"

        if self.mesh_score is True:
            self.pa_mpvpe = AverageMeter()
            self.mpvpe = AverageMeter()
            self.name_pa_mpvpe = f"{name}_PV"
            self.name_mpvpe = f"{name}_V"
    ...
    def get_measures(self, **kwargs) -> Dict[str, float]:
        ...
        measures[f"{self.name_mpjpe}"] = self.mpjpe.avg
        if self.mesh_score:
            measures[f"{self.name_pa_mpvpe}"] = self.pa_mpvpe.avg
            measures[f"{self.name_mpvpe}"] = self.mpvpe.avg
        return measures
    ...
    def __str__(self) -> str:
        score = f"{self.name_pa_mpjpe}(mm): {self.pa_mpjpe.avg * 1000.0 :6.4f} | {self.name_mpjpe}(mm): {self.mpjpe.avg * 1000.0 :6.4f}"
        if self.mesh_score:
            score += f" | {self.name_pa_mpvpe}(mm): {self.pa_mpvpe.avg * 1000.0:6.4f} | {self.name_mpvpe}(mm): {self.mpvpe.avg * 1000.0 :6.4f}"
