...
class Joint3DPCK(_PCKMetric):
    ...
    def _get_predictions(self, preds: Dict, targs: Dict):
        if self.eval_type == "joints_3d_rel":
            ...
            gt_T_c2m = targs["target_cam_extr"].flatten(0, 1)
            # pred_jts_rel = preds["pred_joints_3d_rel"]
            # gt_jts_rel = targs["master_joints_3d_rel"]
            pred_jts_rel = targs['pse_joints_3d'].flatten(0, 2) * 0.2 - targs['pse_joints_3d'].flatten(0, 2)[:, 9:10] * 0.2
            gt_jts_rel = targs["target_joints_3d"].flatten(0, 2) * 0.2 - targs["target_joints_3d"].flatten(0, 2)[:, 9:10] * 0.2
            # pred_joints = preds["master_joints_mvf"]
            # pred_joints = pred_joints.unsqueeze(1).repeat(1, N, 1, 1)
            # pred_joints_in_cam = batch_cam_extr_transf(gt_T_c2m, pred_joints).flatten(0, 1)
            # pred_jts_rel = pred_joints_in_cam - pred_joints_in_cam[:, 9:10]
            # pred_jts_rel = preds["master_joints_mvf"] - preds["master_joints_mvf"][:, 9:10]
            # gt_jts_rel = targs["master_joints_3d"].flatten(0, 1) - targs["master_joints_3d"].flatten(0, 1)[:, 9:10]
            ...
            pred_jts = preds["master_joints_mvf"]
            gt_jts = targs["master_joints_3d"].flatten(0, 1)
...
class Vert3DPCK(_PCKMetric):
    ...
    def __init__(self, **cfg) -> None:
        super().__init__(**cfg)
        self.eval_type = cfg.get("EVAL_TYPE", "verts_3_rel")
        self.reset()
    ...
    def _get_predictions(self, preds: Dict, targs: Dict):
        if self.eval_type == "verts_3d_rel":
            ...
            pred_vts_rel = targs['pse_verts_3d'].flatten(0, 2) * 0.2 - targs['pse_joints_3d'].flatten(0, 2)[:, 9:10] * 0.2
            gt_vts_rel = targs["target_verts_3d"].flatten(0, 2) * 0.2 - targs["target_joints_3d"].flatten(0, 2)[:, 9:10] * 0.2
            # pred_joints = preds["master_joints_mvf"]
            # pred_joints = pred_joints.unsqueeze(1).repeat(1, N, 1, 1)
            # pred_joints_in_cam = batch_cam_extr_transf(gt_T_c2m, pred_joints).flatten(0, 1)
            ...
            # pred_vts_rel = pred_verts_in_cam - pred_joints_in_cam[:, 9:10]
            # pred_vts_rel = preds["master_verts_mvf"] - preds["master_joints_mvf"][:, 9:10]
            # gt_vts_rel = targs["master_verts_3d"].flatten(0, 1) - targs["master_joints_3d"].flatten(0, 1)[:, 9:10]
            return pred_vts_rel, gt_vts_rel, None

        elif self.eval_type == "verts_3d":
            # pred_vts = preds["pred_verts_3d"]
            # gt_vts = targs["master_verts_3d"]
            pred_vts = preds["master_verts_mvf"]
            gt_vts = targs["master_verts_3d"].flatten(0, 1)
            # b = pred_vts.shape[0]
