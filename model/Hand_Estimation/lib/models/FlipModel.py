...
from lib.module_MVT.KPNet_flip import KeypointPredictNet
from lib.module_MVT.CVINet_Flip import CrossViewInteractNet
...
from .heads.MVptEmb_head_MVT import flip_head
...
class TestFlipMultiviewStereo(nn.Module, ModuleAbstract):
    def __init__(self, cfg):
        super(TestFlipMultiviewStereo, self).__init__()
        ...
        self.MVptEmb_head = flip_head(cfg.MV_HEAD)
    ...
    def get_proj_2d(self, pred_joints, T_c2m, K, n_views, affine=None):
        pred_joints = pred_joints.unsqueeze(1).repeat(1, n_views, 1, 1)  # (B_eff, N, 21, 3)
        pred_joints_in_cam = batch_cam_extr_transf(T_c2m, pred_joints)
        pred_joints_2d = batch_cam_intr_projection(K, pred_joints_in_cam)  # (B_eff, N, 21, 2)

        # ====== 新增：将原始 2D 投影点映射到增强后的图像坐标系 ======
        if affine is not None:
            shape = pred_joints_2d.shape  # 记住原始形状 (B_eff, N, 21, 2)
            pred_joints_2d_flat = pred_joints_2d.flatten(0, 1)  # (B_eff * N, 21, 2)

            # 将 affine 变形为 (B_eff * N, 2, 3) 适应不同的 batch size 组合 (如 B 或 B*T)
            affine_flat = affine.reshape(shape[0] * shape[1], -1, 3)[:, :2, :]

            # 补齐齐次坐标进行矩阵乘法 P_aug = affine_2x3 @ [P_orig, 1]^T
            pred_joints_2d_homo = torch.cat([pred_joints_2d_flat, torch.ones_like(pred_joints_2d_flat[..., :1])],
                                            dim=-1)
            pred_joints_2d_aug = torch.bmm(pred_joints_2d_homo, affine_flat.transpose(1, 2))

            pred_joints_2d = pred_joints_2d_aug.reshape(shape)
    ...
    def compute_loss(self, preds, gt):
        ...
        for i in range(preds['joints_preds_list'].shape[0]):
            # 传入 affine 矩阵，使其投影后自动映射到 crop 的增强图空间
            pred_proj_2d = self.get_proj_2d(preds['joints_preds_list'][i], T_c2m, K, N, affine=gt['affine'])
            loss_ref_2d_proj += self.coord_loss(
                confidence * (pred_proj_2d.flatten(0, 1) / (W / 2) - 1),
                confidence * (gt['pse_joints_2d'].flatten(0, 2) / (W / 2) - 1)
            ) * self.cfg.LOSS.KEYPOINTS_2D
        total_loss += loss_ref_2d_proj
        loss_dict['ref_2d_proj_m2p'] = loss_ref_2d_proj

        # Master joints 同理
        pred_proj_master = self.get_proj_2d(preds['master_joints_mvf'], T_c2m, K, N, affine=gt['affine'])
        loss_mano_2d_proj = self.coord_loss(
            confidence * (pred_proj_master.flatten(0, 1) / (W / 2) - 1),
            confidence * (gt['pse_joints_2d'].flatten(0, 2) / (W / 2) - 1)
        ) * self.cfg.LOSS.KEYPOINTS_2D
    ...
    def training_step(self, batch, step_idx, epoch, mode, **kwargs):
        ...
        if step_idx % self.train_log_interval == 0:
            if step_idx % (self.train_log_interval * 25) == 0:  # viz every 10 * interval batches
                ...
                pred_proj_2d = self.get_proj_2d(preds['joints_preds_list'][-1], T_c2m, K, N,affine=batch['affine'])

                pgt_J2d = batch['pse_joints_2d']  # (B, T, N, 21, 2)
                B, T, N, J, _ = pgt_J2d.shape

                # 1. 准备 3x3 逆仿射变换矩阵
                affine_tmp = batch['affine'].reshape(B * T * N, -1, 3)
                if affine_tmp.shape[1] == 2:
                    pad = torch.tensor([[[0.0, 0.0, 1.0]]], dtype=affine_tmp.dtype, device=affine_tmp.device).expand(
                        B * T * N, 1, 3)
                    affine_tmp = torch.cat([affine_tmp, pad], dim=1)
                affine_inv = torch.inverse(affine_tmp)

                # 2. 还原 2D 坐标到原始未裁切图像空间
                pgt_J2d_flat = pgt_J2d.reshape(B * T * N, J, 2)
                pgt_J2d_homo = torch.cat([pgt_J2d_flat, torch.ones_like(pgt_J2d_flat[..., :1])], dim=-1)
                pgt_J2d_orig = torch.bmm(pgt_J2d_homo, affine_inv.transpose(1, 2))[..., :2]
                pgt_J2d_orig = pgt_J2d_orig.reshape(B * T, N, J, 2)

                # 3. 在原始物理空间中进行真实三角化
                pse_vis = batch['pse_joints_vis'].reshape(B * T, N, J)
                pse_joints = batch_triangulate_dlt_cfd_torch(pgt_J2d_orig, K, T_c2m, pse_vis)

                # 4. 投影回 2D 增强空间进行可视化：
                # 传入 batch['affine']，利用我们刚修改好的 get_proj_2d 自动完成【原始投影 -> 增强坐标】的转换
                pse_proj_2d = self.get_proj_2d(pse_joints, T_c2m, K, N, affine=batch['affine'])
                ...
                for i in range(T):
                    ...
                    img_array_hm = draw_batch_joint_images(pred_J2d_hm, pgt_J2d, img_toshow, step_idx)
                    self.summary.add_image(f"img_hm_{view_id}/viz_joints_2d_val_seq_{i}", img_array_hm, step_idx,
                                           dataformats="NHWC")

                    img_array = draw_batch_joint_images(pred_proj, pgt_J2d, img_toshow, step_idx)
                    self.summary.add_image(f"img_master_{view_id}/vis_joints_2d_val_seq_{i}", img_array, step_idx,
                                           dataformats="NHWC")
