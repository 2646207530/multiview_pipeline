...
class ConvNeXt(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, LayerNorm):
            device = module.weight.device  # 确保保持和原始 weight 一致
            module.weight = nn.Parameter(torch.ones(module.normalized_shape, device=device))
            module.bias = nn.Parameter(torch.zeros(module.normalized_shape, device=device))
        if isinstance(module, (nn.Conv2d, nn.Linear)):
            torch.nn.init.trunc_normal_(module.weight, std=0.02)
            nn.init.constant_(module.bias, 0)

    def forward_features(self, x: Tensor , masks: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
    ...
    def forward_features_seq_out(
        ...
        if return_class_token:
            return tuple(outputs), tuple(class_tokens)
    ...
    def _change_res(self, in_channels):
        self.downsample_layers[0] = nn.Sequential(
            nn.Conv2d(in_channels, self.dims[0], kernel_size=2, stride=2),
            LayerNorm(self.dims[0], eps=1e-6, data_format="channels_first")
        )

    @classmethod
    def from_name(cls, model_name, in_channels=3, high_res=False):
        params = convnext_sizes[model_name.split('_')[1]]
        model = cls(depths=params['depths'], dims=params['dims'])
        load_pretrained_weights(model, model_name, weights_path=model_urls[model_name])
        if in_channels != 3:
            model._change_in_channels(in_channels)
        if high_res:
            model._change_res(in_channels)
        return model

    @classmethod
    def from_pretrained(cls, model_name, weights_path=None,
...
def load_pretrained_weights(model, model_name, weights_path=None, verbose=True):
...
home_dir =  '.'
model_urls = {
    "convnext_tiny": "%s/dinov3_convnext_tiny_pretrain_lvd1689m-21b726bb.pth" %(home_dir),
    "convnext_small": "%s/dinov3_convnext_small_pretrain_lvd1689m-296db49d.pth"%(home_dir),
    "convnext_base": "%s/dinov3_convnext_base_pretrain_lvd1689m-801f2ba9.pth"%(home_dir),
    "convnext_large": "%s/dinov3_convnext_large_pretrain_lvd1689m-61fa432d.pth"%(home_dir),
}
...
class ConvNeXt_Tiny(ConvNeXt):
    def __init__(self, cfg):
        params = convnext_sizes["tiny"]
        super(ConvNeXt_Tiny, self).__init__(depths=params['depths'], dims=params['dims'])
        self.name = "convnext_tiny"
        # 兼容你的 cfg 参数读取方式
        if getattr(cfg, "PRETRAINED", False):
            load_pretrained_weights(self, self.name, weights_path=model_urls[self.name])

@BACKBONE.register_module()
class ConvNeXt_Small(ConvNeXt):
    def __init__(self, cfg):
        params = convnext_sizes["small"]
        super(ConvNeXt_Small, self).__init__(depths=params['depths'], dims=params['dims'])
...
class ConvNeXt_Base(ConvNeXt):
    def __init__(self, cfg):
        params = convnext_sizes["base"]
        super(ConvNeXt_Base, self).__init__(depths=params['depths'], dims=params['dims'])
...
class ConvNeXt_Large(ConvNeXt):
    def __init__(self, cfg):
        params = convnext_sizes["large"]
        super(ConvNeXt_Large, self).__init__(depths=params['depths'], dims=params['dims'])
...
if __name__ == '__main__':
    img = torch.ones([32, 1, 128, 128])
    model = ConvNeXt.from_name('convnext_tiny', in_channels=1)
    print(model.get_intermediate_layers(img, n=4))