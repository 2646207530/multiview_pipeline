...
class FlipDataset(HDataset):
    ...
    def __init__(self, cfg):
        ...
        # 训练时只走左手 (师兄默认), 推理时打开这个开关让右手也进 mapping;
        # 否则 inference 出来只有左手, 右手全丢.
        self.include_right_hand = bool(cfg.get("INCLUDE_RIGHT_HAND", False))
    ...
    def build_mapping(self):
        ...
        for pesudo_file in pesudo_files:
            ...
            if hand_id == 0 and not self.include_right_hand:
                continue
    ...
    def getitem_test(self, idx):
        ...
        results["is_left"] = np.array([flip_hand], dtype=np.bool_)

        DEBUG_VIS = False
        if DEBUG_VIS and random.random() < 0.1:
            import os
            save_dir = './debug_vis_flip'
            os.makedirs(save_dir, exist_ok=True)

            orig_pts = joints_2d.copy()
            aug_pts = results['pse_joints_2d'].copy()

            # 直接提取，不需要再判断 shape 或 vstack！
            affine_inv = results['affine_inv']

            aug_pts_homo = np.concatenate([aug_pts, np.ones((aug_pts.shape[0], 1))], axis=-1)
            restored_pts = (affine_inv @ aug_pts_homo.T).T[:, :2]

            vis_img = image_ori.copy()
            for pt in orig_pts:
                cv2.circle(vis_img, (int(pt[0]), int(pt[1])), 6, (0, 255, 0), -1)
            for pt in restored_pts:
                cv2.circle(vis_img, (int(pt[0]), int(pt[1])), 3, (0, 0, 255), -1)

            cv2.imwrite(os.path.join(save_dir, f'{idx}_restored_isLeft_{flip_hand}.jpg'), vis_img)

        return results


@DATASET.register_module()
class FlipDatasetMultiView(torch.utils.data.Dataset):
    def __init__(self, cfg):
        super().__init__()
    ...
    def _single_view_test(self):
        cfg_train = dict(
            ...
            # 透传 INCLUDE_RIGHT_HAND, 否则内部 FlipDataset 默认 false → 右手被滤掉.
            INCLUDE_RIGHT_HAND=self.cfg.get("INCLUDE_RIGHT_HAND", False),
        )
        cfg_val = cfg_train.copy()
        cfg_val["DATA_SPLIT"] = "val"

        return FlipDataset(CN(cfg_train)), FlipDataset(CN(cfg_val))
...
@DATASET.register_module()
class FlipMultiViewTempo(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for t in range(self.window_size):
            ...
            sample_need = [
                ...
                'cam_num', 'file_num', 'rotation', 'cfd', "extr_prerot","image_path","is_left"
            ]
