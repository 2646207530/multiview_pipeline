...
from ..utils.transform_seq import fit_ortho_param, ortho_project
...
class HDataset(torch.utils.data.Dataset, ABC):
    ...
    @staticmethod
    def get_pse_label(self):
        pass

    @staticmethod
    def get_mano_results(self, idx):
        pass
    # @abstractmethod
    ...
    def getitem_3d(self, idx):
        ...
        verts_uvd = self.get_verts_uvd(idx)
        joints_uvd = self.get_joints_uvd(idx)
        pseudo_joints, pseudo_verts, pseudo_uv, _ = self.get_pse_label(idx)
        # _, _, pseudo_uv, _ = self.get_mano_results(idx)
        image_path = self.get_image_path(idx)
        ...
        raw_size = [image.shape[1], image.shape[0]]  # (W, H)
        joints_vis = self.get_joints_2d_vis(joints_2d=joints_2d, raw_size=raw_size)
        pse_vis = self.get_joints_2d_vis(joints_2d=pseudo_uv, raw_size=raw_size)
        flip_hand = True if hand_side != self.sides else False
        ...
        if flip_hand:
            ...
            pseudo_uv = self.flip_2d(pseudo_uv, centerX=raw_size[0])
            pseudo_joints = self.flip_2d(pseudo_joints, centerX=raw_size[0])
            pseudo_verts = self.flip_2d(pseudo_verts, centerX=raw_size[0])
            image = image[:, ::-1, :]
        ...
        label = {
            ...
            "cam_intr": cam_intr,
            "pseudo_2d": pseudo_uv,
            "pse_joints": pseudo_joints,
            "pse_verts": pseudo_verts,
            "pseudo_vis": pse_vis,
            # "pseudo_cfd": pseudo_cfd,
            "joints_2d": joints_2d,
            "joints_3d": joints_3d,
            "verts_3d": verts_3d,
            "joints_vis": joints_vis,
    ...
    def __getitem__(self, idx):
        if self.data_mode not in ["2D", "3D", "UVD", "UVD_ortho","Test"]:
