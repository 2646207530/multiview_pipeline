...
class Model_RGBD(object):
    ...
    def estimate_pose_RGBD(self, input):
        ...
        # RGB Aug
        # crop_RGB, img2bb_trans_rgb, bb2img_trans_rgb, _, _, = augmentation(img_rgb, bbox, self.phase, exclude_flip=False)
        ...
        # cam_para = (906.96, 906.79, 956.75, 547.23)
        img, pcl, com3D, M, cube = self.process_depth(self.cube, img_d, center)  # trans=M
        temp_img = (img.permute(1, 2, 0) + 1) / 2 * 255.0
        temp_img = cv2.cvtColor(temp_img.numpy().astype(np.uint8), cv2.COLOR_GRAY2RGB)
        # print(temp_img.shape)
        # self.videoWriter.write(temp_img.astype(np.uint8))
        ##cv2.imshow('crop_depth', temp_img)
        # cv2.imshow('crop_depth', img)
        # cv2.imshow(str(p), depth)
        ##cv2.waitKey(1)  # 1 millisecond

        temp_img_rgb = (img_rgb[0] * 255).permute(1, 2, 0)
        temp_img_rgb = cv2.cvtColor(temp_img_rgb.numpy().astype(np.uint8), cv2.COLOR_BGR2RGB)
        # print(temp_img.shape)
        # self.videoWriter.write(temp_img.astype(np.uint8))
        ##cv2.imshow('crop_RGB', temp_img_rgb)
        ##cv2.waitKey(1)  # 1 millisecond
        ...
        # crop_depth, pcl, M,  = \
        #     self.depthloader.augmentation_depth(img_d, cube ,center, self.cam_para)

        # IMG2PCL 生成点云
        # pcl = self.depthloader.generatePCL(img.squeeze(0),com3D,cube, M, self.cam_para)

        # img_rgb, img, pcl, center, M, cube, cam_para = data
        ...
        for index, stage_type in enumerate(self.config.stage_type):
            if stage_type == 1 and len(joint_xyz_list)!=0:
                #com3D = joint_xyz_list[0][0].mean(0)
                print(com3D)
                print(center)
            if stage_type == 1:
        ...
        joint_xyz_world = joint_xyz_list[2] * cube.unsqueeze(1) / 2 + center.unsqueeze(1)

        #joint_xyz_world = joint_xyz_world.detach().cpu().numpy()[:, MANO2HO3D, :]
        joint_xyz_world = joint_xyz_world.detach().cpu().numpy()
        joint_xyz_world *= np.array([1, -1, -1]) / 1000

        # uvd2img
        MANO2HANDS = [0, 13, 1, 4, 10, 7, 14, 15, 20, 2, 3, 16, 5, 6, 17, 11, 12, 19, 8, 9, 18]
        #joint_uvd = joint_uvd_list[0][:, MANO2HANDS, :].squeeze(0)

        joint_uvd = joint_uvd_list[2].squeeze(0)
        joint_uvd[:, 2] = (joint_uvd[:, 2] * cube[0][0] / 2) + center[0][2]
        #joint_uvd[:, :2] = (joint_uvd[:, :2] + 1) * self.img_size // 2
        # back to original image
        coord_uvd_full = joint_uvd.clone()

        coord_uvd_full = self.transformPoints2D(coord_uvd_full.cpu().data.numpy(), np.linalg.inv(M.cpu().data.numpy()))
        img_show = vis_tool.draw_2d_pose(img[0], joint_uvd, 'dexycb')

        rgb_show = vis_tool.draw_2d_pose_rgb(img_rgb[0], joint_uvd, 'dexycb')
        rgb_show = cv2.cvtColor(rgb_show, cv2.COLOR_BGR2RGB)


        meta_info = {
            'crop_img': temp_img_rgb,
            'crop_img_d': temp_img,
            'K': None,
            'img2bb_trans': M,
    ...
    def get_center_from_bbx(self, depth, bbx, upper=1500, lower=171):
        ...
        # x_min, y_min, w, h = bbx #bbox的前两个值是左上角坐标
        # x_c, y_c = x_min + w / 2, y_min + h / 2 #bbox的中心值
        # #计算质心时，不是考虑box的中点，要考虑中点周围的一部分范围
        # x_min, y_min = x_c - 5, y_c - 5 #考虑中心周围10mm的范围
        # x_max, y_max = x_c + 5, y_c + 5
        # #depth_center = copy.deepcopy(depth)
        # # depth_center[:, :int(x_min)] = 0 #范围外的区域，深度值设为0
        # # depth_center[:int(y_min), :] = 0
        # # depth_center[:, int(x_max):] = 0
        # # depth_center[int(y_max):, :] = 0
    ...
    def process_depth(self, cube_size, depth, center):
        ...
        # center_uvd = self.joint3DToImg(center)
        center_uvd = center
    ...
    def Crop_Image_deep_pp(self, depth, com, size, dsize, paras):
        ...
        # print(scale)
        # print(off)
        # print(np.dot(off, np.dot(scale, trans)))
        # [[    0.18551           0     -262.68][          0     0.18551     -67.525][          0           0           1]]

        # cropped_re = np.dot(np.linalg.inv(np.dot(off, np.dot(scale, trans))),cropped)
        # print(cropped_re.shape)
...
if __name__ == '__main__':
    ...
    roots = ['/home/cyc/pycharm/SDF_wild/sdf_video/wood_0/','/home/cyc/pycharm/SDF_wild/sdf_video/wood_1/','/home/cyc/pycharm/SDF_wild/sdf_video/scissor_0/']
    model_RGBD = Model_RGBD(opt)
    for root in roots:
        for file_name in os.listdir(os.path.join(root,'rgb')):
            rgb = cv2.imread(os.path.join(root,'rgb',file_name[:-4]+'.png'))

            depth = cv2.imread(os.path.join(root,'depth',file_name[:-4]+'.png'), cv2.IMREAD_ANYDEPTH)
        #bbox = [0.50911456*1920-0.10885417*1920/2,0.32638890*1080-0.24166666*1080/2,0.10885417*1920,0.24166666*1080]
            bbox = np.loadtxt(os.path.join(root,'bbox',file_name[:-4]+'.txt'))
            bbox[0]*=1920
            bbox[1] *= 1080
            bbox[2] *= 1920
            bbox[3] *= 1080
            ...
            #print(result['joint_uvd'])

            os.makedirs(os.path.join(root, 'debug'), exist_ok=True)

            cv2.imwrite(os.path.join(root,'debug',file_name[:-4]+'_posed.png'), cv2.cvtColor(np.uint8(meta_info['pose_rgb']), cv2.COLOR_BGR2RGB))
            cv2.imwrite(os.path.join(root,'debug',file_name[:-4]+'_posed_d.png'), meta_info['pose_d'])
            os.makedirs(os.path.join(root,'joints_kpfusion'),exist_ok=True)

            path = os.path.join(root,'joints_kpfusion',file_name[:-4]+'.npz')
            np.savez(path,joint_xyz=result['joint_xyz'])
