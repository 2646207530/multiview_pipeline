...
sys.path.append('..')
from software.KeypointFusion.util import vis_tool,object_transform
from software.KeypointFusion.dataloader.webuser.smpl_handpca_wrapper_HAND_only import load_model
...
class loader(Dataset):
    ...
    def get_trans_points(self, joints, M):
        ...
        joints_mat = torch.cat((joints[:, :, 0:2], torch.ones(joints.size(0), joints.size(1), 1).to(device)), dim=-1).float()
...
if __name__ == "__main__":
    ...
    trainData = DexYCBDataset('s0', 'train', '/home/cyc/pycharm/data/hand/',aug_para=[10, 0.2, 180], input_modal='RGBD')
