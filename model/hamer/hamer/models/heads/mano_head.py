...
# model/hamer 目录（含 _DATA/data），与机器无关；heads->models->hamer 共 3 级
_HEAD_DIR = os.path.dirname(os.path.abspath(__file__))
_HAMER_ROOT = os.path.normpath(os.path.join(_HEAD_DIR, '..', '..', '..'))
MEAN_PARAMS_PATH = os.path.abspath(os.path.join(_HAMER_ROOT, '_DATA', 'data', 'mano_mean_params.npz'))
...
class MANOTransformerDecoderHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # 始终用基于 __file__ 的绝对路径，避免 config 里相对路径随 cwd 解析错误
