...
def generate_image_patch_cv2(img: np.array, c_x: float, c_y: float,
    ...
    if img is None or img.shape[0] == 0 or img.shape[1] == 0:
        raise ValueError("Input image is empty or has zero dimensions!")
    
    if patch_width <= 0 or patch_height <= 0:
        raise ValueError(f"Invalid patch size: width={patch_width}, height={patch_height}")
    ...
    trans = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, patch_width, patch_height, scale, rot)
    inv_trans = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, patch_width, patch_height, scale, rot)
    if trans is None or trans.shape != (2, 3):
        raise ValueError(f"Affine transformation matrix `trans` is invalid! Got: {trans}")
    #print(f"Image shape: {img.shape}, trans: {trans}, patch size: ({patch_width}, {patch_height})")
    img_patch = cv2.warpAffine(img, trans, (int(patch_width), int(patch_height)), 
                        flags=cv2.INTER_LINEAR, 
                        borderMode=border_mode,
                        borderValue=border_value,
                )
    # img_patch_inv = cv2.warpAffine(img_patch, inv_trans, (int(img_width), int(img_height)), 
    #                     flags=cv2.INTER_LINEAR, 
    #                     borderMode=border_mode,
    #                     borderValue=border_value,
    #             ).astype(np.uint8)
...
#                 bbox_size: float, image_size: int, do_flip: bool):
def get_example(img_path: Union[str, np.ndarray], center_x: float, center_y: float,
