...
# 与 config/hamer_config.py 一致，使用仓库相对路径或 FBS_MODEL_DIR
ckpt_path = hamer_opt.ckpt_path
model_cfg = hamer_opt.model_cfg
HAMER_ONNX_CKPT_PATH = hamer_opt.onnx_path
...
class hamer_inference():
    ...
    def estimate_from_rgb(self, img_0, detections, k_real=None):
            ...
            # do_flip: 1.0=左手(已翻转), 0.0=右手. flip_correction: 右=+1, 左=-1
            # (公式 1 - 2*do_flip; 用来把 do_flip=1 的 hand 在 X 上镜像回来)
            flip_correction = 1.0 - 2.0 * do_flip.view(-1)

            # 处理 3D 关键点 (翻转处理)
            # 旧 bug: 用 do_flip 直接乘, 右手 (do_flip=0) 会把 X 归零, 导致
            # pred_keypoints_2d_full 全部投到主点附近, 多视角 pseudo label 全错位.
            # 正确做法: 用 flip_correction (+/- 1), 把左手镜像回真实空间.
            pred_keypoints_3d = out['pred_keypoints_3d'].float()
            pred_keypoints_3d[:, :, 0] = pred_keypoints_3d[:, :, 0] * flip_correction.view(-1, 1)

            # =======================================================
            # === 针对左手（flipped），反转 cam_bbox 的 x 偏移量 ===
            # =======================================================
