...
# /tmp/gradio 通常被别的用户占着 (PermissionError), 把 gradio 的 temp / cache
# 指到本仓库下的 .gradio_cache, 必须在 import gradio 之前设.
_GRADIO_TMP = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".gradio_cache")
os.makedirs(_GRADIO_TMP, exist_ok=True)
os.environ.setdefault("GRADIO_TEMP_DIR", _GRADIO_TMP)
os.environ.setdefault("GRADIO_CACHE_DIR", _GRADIO_TMP)
