...
class Detector():
    ...
    def plot_bbox(self, img, bboxs, size=3):
        ...
        for det in bboxs:
            for *xyxy, conf, cls in reversed(det):
                ...
                #图像反转后左右手识别翻转
                if 'Rhand' in names[int(cls)]:
                    label = f'Lhand {conf:.2f}'
                if 'Lhand' in names[int(cls)]:
                    label = f'Rhand {conf:.2f}'

                # label = f'{names[int(cls)]} {conf:.2f}'
                color = fixed_colors[int(cls) % len(fixed_colors)]
                plot_one_box(xyxy, img, label=label, color=color, line_thickness=size)
        return img

    def cut_to_bbox(self, img, bboxs):
        # Get names and colors
        names = self.model.module.names if hasattr(self.model, 'module') else self.model.names
        colors = [[random.randint(0, 255) for _ in range(3)] for _ in names]
