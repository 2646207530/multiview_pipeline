...
def training(config):
    ...
    body_model_r = MANO(model_path='/mnt/sda2/lxy/arctic/unpack/body_models/mano/', flat_hand_mean=True).cuda()
    body_model_l = MANO(model_path='/mnt/sda2/lxy/arctic/unpack/body_models/mano/', is_rhand=True, flat_hand_mean=True).cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        data_idx = iteration-1
        if data_idx == len(scene.train_dataset)-1 :
            break
        ...
        render_mesh_pkg = render_mesh(render_pkg["updated_camera"], iteration, scene, body_model_r, body_model_l, vis=True)
