...
from models.pose_correction.pose_correction import get_pose_correction
...
def training(config):
    ...
    if config.load_ckpt:
        scene.load_checkpoint(config.load_ckpt, strict=False)

    scene.converter.pose_correction = get_pose_correction(config.model.deformer, scene.smpl_data).cuda()
    scene.converter.set_optimizer(pose_only=True)
    scene.train()
    print("training_samples:", len(scene.train_dataset))

    for sub_id in gaussians_hand_group:
        gaussians_hand_group[sub_id]['right'].training_setup(opt, template=False)
        gaussians_hand_group[sub_id]['left'].training_setup(opt, template=False)
    for obj_id in gaussians_obj_group:
        gaussians_obj_group[obj_id].training_setup(opt, template=False)

   
    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]
    background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

    body_model_r = MANO(model_path='/mnt/sda2/lxy/arctic/unpack/body_models/mano/', flat_hand_mean=False).cuda()
    body_model_l = MANO(model_path='/mnt/sda2/lxy/arctic/unpack/body_models/mano/', is_rhand=False, flat_hand_mean=False).cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_mesh_pkg = render_mesh(render_pkg["updated_camera"], iteration, scene, body_model_r, body_model_l)
        ...
        if iteration==1 or iteration % 100 == 0:
            ...
            import cv2
            cv2.imwrite('./output/debug/full_gt_image.png',
                                    cv2.cvtColor(np.uint8(full_gt_image.permute(1, 2, 0).detach().cpu().numpy() * 255),
                                                 cv2.COLOR_BGR2RGB))
            cv2.imwrite('./output/debug/full_image.png',
        ...
        with torch.no_grad():
            ...
            # Optimizer step
            scene.converter.optimize(iteration)
...
def validation(iteration, testing_iterations, testing_interval, scene: Scene, evaluator, renderArgs):
    ...
    for idx, data in enumerate(scene.train_dataset):
        ...
        render_pkg = render(data, iteration+idx, scene, *renderArgs, compute_loss=True,
                            return_opacity=True, novel_data=novel_cacmera)
        examples = []
...
@hydra.main(version_base=None, config_path="configs", config_name="config_image")
def main(config):
    # print(OmegaConf.to_yaml(config))
    OmegaConf.set_struct(config, False)  # allow adding new values to config
    # print(config.name)
    config.exp_dir = config.get('exp_dir') or os.path.join('/mnt/sda1/lxy/SportGS_results/', config.dataset._YCB_CLASSES[0],config.name)
    ...
    if not config.wandb_disable:
        try:
            shutil.copyfile('train_image.py', config.exp_dir + '/code/train_image.py')
    ...
    swanlab_log = os.path.join('/mnt/sda1/lxy/SportGS_results/', 'swanlab', 'SportGS')
    os.makedirs(swanlab_log, exist_ok=True) 
    swanlab.init(
        name=wandb_name,
        project='SportGS_1221',
