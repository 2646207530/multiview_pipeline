...
def training(config):
    ...
    scene = Scene(config, gaussians_hand_group, gaussians_obj_group, config.exp_dir, freeze_pose=True)
    scene.train()
    #scene.eval()
    ...
    opt_debug_stage = config.dataset.get('opt_debug_stage', 'fc')
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        if iteration == 5 or iteration == 15000:
            scene.converter.pose_correction.export('finetune.npy',
        ...
        render_mesh_pkg, loss_reg_mesh = render_mesh(render_pkg["updated_camera"], iteration, scene, body_model_r, body_model_l, mano_finger_labels, 
                                      vis=(iteration <5 or iteration%1000==0 or iteration>=14920), finetune=True)
