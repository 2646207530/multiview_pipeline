...
class ArcticDataset(Dataset):
    ...
    def _load_data(self, cfg):
        ...
        if self.split == 'train':
            data_p = os.path.join(
                f"/mnt/sda1/lxy/dataset/ARCTIC_demo/splits/train/{cfg.split_train}.npy"
            )
        else:
            self.train_view_id = int(cfg.split_train.split('_')[-1])
            data_p = os.path.join(
                f"/mnt/sda1/lxy/dataset/ARCTIC_demo/splits/test/{cfg.split_test}.npy"
            )
        ...
        with open("/mnt/sda1/lxy/dataset/ARCTIC_demo/meta/misc.json", "r") as f:
            misc = json.load(f)
    ...
    def get_smpl_data(self):
        ...
        for idx, imgname in enumerate(self.imgnames):
            ...
            vidx = int(image_idx.split(".")[0]) - self.ioi_offset[sid]
            data_params = self.data[f"{sid}/{seq_name}"]["params"]
            ...
            world2cam = np.array(self.world2cam[sid][int(view_idx) - 1])
    ...
    def __getitem__(self, idx):
        ...
        imgname = imgname.replace('./arctic_data/data/images/', 'images/')
        img_name = os.path.join(self.root_dir, imgname)
        
       
        seg_path = img_name.replace('/images/', '/seg/').replace('jpg', 'png')
